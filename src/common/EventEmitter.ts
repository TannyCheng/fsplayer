...
export class EventEmitter {
  ...
  emit(evt: string, a1?: any, a2?: any, a3?: any, a4?: any, a5?: any): boolean {
    ...
    if (listeners.fn) {
      ...
      for (i = 0; i < length; i++) {
        if (listeners[i].once) this.off(evt, listeners.fn, undefined, true);
  ...
  on(evt: string, fn: Function, context?: any, once = false): Disposable {
    ...
    if (!this.events[evt]) (this.events[evt] = listener), this.eventsCount++;
    else if (!this.events[evt].fn) this.events[evt].push(listener);
    else this.events[evt] = [this.events[evt], listener];

    return { dispose: this.off.bind(this, evt, fn) };
  }

  once(evt: string, fn: Function, context?: any) {
